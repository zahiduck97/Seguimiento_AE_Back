...
router.get('/', async(req, res) => {
    const db = await pool.query(`SELECT * FROM TipoServicio WHERE activo = 1 ORDER BY id DESC`).catch(e => {
...
router.put('/activo/:id', async(req, res) => {
    ...
    const db = await pool.query(`UPDATE TipoServicio SET activo = 0 WHERE id = ${id}`).catch(e => {
